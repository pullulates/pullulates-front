  ...
  >
    <a-form :form="form">
      <a-form-item hidden="true">
        <a-input type="hidden" v-decorator="['jobId']" />
      </a-form-item>
      <a-form-item
        label="任务名称"
        :labelCol="labelCol"
...
export default {
  ...
  methods: {
    show (record, jobStatus) {
      this.mdl = Object.assign({}, record)
      this.visible = !this.visible
      this.confirmLoading = true
      this.jobStatus = jobStatus
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'jobId', 'name', 'groupName', 'invokeTarget', 'cron', 'status', 'remark'))
        this.cron = record.cron
      })
